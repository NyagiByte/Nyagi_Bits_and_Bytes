...
public class ModOPAMetals {
    ...
    private static void addOPAMetal(String id, int opaTint, int frothTint, int metalTint){
        ...
        ModRegistries.ITEMS_LIST.add(new ItemInfo("sample_pure_"+id, Utils.Tab.ELEMENTS).chem(ItemInfo.ChemType.SOLID, "pure_"+id).tint(metalTint));
        ModRegistries.ITEMS_LIST.add(new ItemInfo("pure_"+id+"_dust", Utils.Tab.ELEMENTS).chem(ItemInfo.ChemType.DUST, "pure_"+id).tint(metalTint));
        ModRegistries.ITEMS_LIST.add(new ItemInfo("pure_"+id+"_ingot", Utils.Tab.ELEMENTS).chem(ItemInfo.ChemType.INGOT, "pure_"+id).tint(metalTint));
        ModRegistries.ITEMS_LIST.add(new ItemInfo("pure_"+id+"_plate", Utils.Tab.ELEMENTS).chem(ItemInfo.ChemType.PLATE, "pure_"+id).tint(metalTint));
        ModRegistries.ITEMS_LIST.add(new ItemInfo("float_separated_"+id+"_dust", Utils.Tab.ELEMENTS).chem(ItemInfo.ChemType.FLOAT_DUST, "pure_"+id).tint(metalTint));

        BlockInfo block = new BlockInfo("pure_"+id+"_block", SoundType.METAL, 4f, true, Utils.Tab.ELEMENTS);
        ModRegistries.BLOCKS_LIST.add(block.tint(metalTint));
    }

    private static void addOPAMetal(String id, int opaTint, int frothTint){
        PURE_METALS.add(id);
        Color rgb = new Color(opaTint); //TODO: FIX THE TINTS
