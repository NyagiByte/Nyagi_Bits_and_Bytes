...
public class ModClientEvents {
    ...
    public static void onItemColorHandlerEvent(RegisterColorHandlersEvent.Item e){
        ModRegistries.ITEMS.getEntries().stream().map(RegistryObject::get).toList().forEach(item -> {
            if(item instanceof NBNBBlockItem tinted) e.register(tinted::getColor, tinted);
    ...
    public static void onBlockColorHandlerEvent(RegisterColorHandlersEvent.Block e){
        ModRegistries.BLOCKS.getEntries().stream().map(RegistryObject::get).filter(block -> block instanceof NBNBBlock tinted).map(block -> (NBNBBlock) block).filter(NBNBBlock::hasTint).toList().forEach(tintedBlock -> {
